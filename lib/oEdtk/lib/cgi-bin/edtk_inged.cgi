...
use oEdtk::EDM		qw(edm_process edm_import);
use File::Basename;
use File::Copy;
...
my $fh  	= $req->upload('EDM_FILENAME');
my $IDLDOC 	= oe_ID_LDOC;
my $ext 	= $req->upload('EDM_FILENAME');
$ext		=~s/\.(\w+)$/$1/;

if (defined $req->param('EDM_FILENAME') && $req->param('EDM_FILENAME') ne "") {
	eval {
